...
create or replace package body WORK_DAY_OPT is
    ...
    begin 
      ...
      if dayD='1' or dayD='7' then
         select count(1) into nTemp from OA_WORK_DAY where workday=currDate and daytype='B';
         if nTemp > 0 then
            nRes := 1;
         end if;
       else
         select count(1) into nTemp from OA_WORK_DAY where workday=currDate and daytype='A';
         if nTemp = 0 then
            nRes := 1;
         end if;
       end if;
 ...
 /**
   计算 工作日后的时间，
   这个最好重构一下 OA_WORK_DAY 表，记录所有的天是否是工作日，并对工作日排序，这样就可以很容的查找到对应的日期了。
   **/
   ...
   begin
     ...
     from OA_WORK_DAY where workday >=currDate and workday<nextWorkDate;
