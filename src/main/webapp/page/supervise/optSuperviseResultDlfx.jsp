	...
	<s:form action="%{actionName}" method="post" namespace="/supervise"
		...
		<fieldset style="display: block; padding: 10px;">
			...
			<c:if
				...
				<c:set var="titles">
					<c:if test="${object.bjType eq '1'}">${object.apply.transactAffairName}办件</c:if>
					<c:if test="${object.bjType eq '3' and not empty object.apply.no}">对于办件(${object.apply.transactAffairName})投诉</c:if>
					<c:if test="${object.bjType eq '2'}">${object.punish.content}案件</c:if>
					<c:if test="${object.bjType eq '3' and not empty object.punish.no}">对于案件(${object.punish.content})投诉</c:if>
				</c:set>
			...
			<table border="0" cellpadding="0" cellspacing="0" id="tb"
				...
				<c:if test="${ not empty object.complaintid }">
					<tr id="confirm">
						<td class="addTd" width="13%">投诉是否属实<font color="red">*</font></td>
						<td align="left" width="87%"><select id="isconfirm"
							name="confirm">
								<c:forEach var="row" items="${cp:LVB('TSDCJG')}">
					...
					<input type="hidden" name="operatorResult" id="operatorResult"
						value="T" />
				</c:if>
				<tr id="isExternal">
