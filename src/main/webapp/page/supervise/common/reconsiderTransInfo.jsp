...
<body>
<s:form action="%{actionName}" method="post" namespace="/supervise" id="superviseForm" target="_parent" onsubmit="return checkForm();">
<input type="hidden" id="flowInstId" name="flowInstId" value="${flowInstId}" />
<input type="hidden" id="reconsiderid" name="reconsiderid"  value="${reconsiderid}" />
<input type="hidden" id="reconsiderId" name="reconsiderId"  value="${reconsiderid}" />
...
<input type="hidden" id="transidea" name="transidea" value="">
			...
			<p align="center" style="font-size: 20; color: #01A6E0;">关于对<c:if test="${object.bjType=='1' or (object.bjType=='3' and not empty object.apply.no)}">${object.apply.transactAffairName}办件</c:if><c:if test="${object.bjType=='2' or (object.bjType=='3' and not empty object.punish.no)}">${object.punish.content}处罚</c:if>的行政复议申请		
			</p>
			</c:if>
<fieldset style=" display: block; padding: 10px;">
			...
			<c:if test="${moduleParam.hasIdea eq 'T' }">
					...
					<td align="left" width="87%">
						...
						<option VALUE="" >---请选择---</option>
							<c:forEach var="row" items="${cp:LVB(moduleParam.ideaCatalog)}">
								<option value="${row.value}" label="${row.label}" <c:if test="${operatorResult==row.value}">selected="selected"</c:if>>
				...
				<tr>
					<td class="addTd" width="13%" id="ideacontent">${moduleParam.ideaContent}<font color="red">*</font></td>
					<td align="left" width="87%">
						<s:textarea id="operatorOpinion"
								name="operatorOpinion" style="width:100%; height:40px;" value="%{object.operatorOpinion}"/>
					</td>
				</tr>
								        <!-- 办件角色 -->
