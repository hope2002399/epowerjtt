...
<html>
	...
	<body>
		...
		<legend class="ctitle" style="width:auto;font:·16px">督查督办过程信息</legend>
			<table border="0" cellpadding="1" cellspacing="1" class="viewTable">
				...
				<tr>
					...
					<td align="left"><fmt:formatDate value="${supInfo.receiptDate}" pattern="yyyy-MM-dd hh:mm:dd"/> &nbsp;</td>
			...
			<c:if test="${supInfo.superviseCode eq null}">
			<fieldset style="padding:10px;">
		<legend class="ctitle" style="width:auto;font:·16px">督办基本信息</legend>
			...
			<s:textfield name="supervisionYear" value="%{object.supervisionYear}" type="hidden"/>
			<s:textfield name="no" value="%{object.no}" type="hidden"/>
			<s:textfield name="supervisionMonth" value="%{object.supervisionMonth}" type="hidden"/>
			<table cellpadding="0" cellspacing="0" class="viewTable">
				<tr height="22">
					<td class="addTd">被督办部门:</td>
					<td align="left"><s:textfield name="orgId" value="%{object.orgId}" type="hidden"/>${cp:MAPVALUE("depno",object.orgId)} </td>
				</tr>
				<tr height="22">
			...
			<c:if test="${supInfo.superviseCode ne null}"><table cellpadding="0" cellspacing="0" class="viewTable">
				<tr height="22"><td align="center"><input type="button" class="btn" value="返回" onclick="javascript:history.go(-1);" /></td></tr></table></c:if>
