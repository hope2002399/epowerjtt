...
<html>
		...
		<legend class="ctitle" style="width:auto;font:·16px">督查督办过程信息</legend>
			<table border="0" cellpadding="1" cellspacing="1" class="viewTable">
				...
				<tr>
					...
					<td colspan="3"><fmt:formatDate value="${supInfo.receiptDate}" pattern="yyyy-MM-dd hh:mm:dd"/> &nbsp;</td>
				</tr>
			</table>	
			</fieldset>
			<fieldset>
		<legend class="ctitle" style="width:auto;">督查督办结果信息</legend>
		 <s:form  action="supInfoBasicDlfx" namespace="/supervise" style="margin-top:0;margin-bottom:5" id="supInfoBasicDlfxForm" method="post">
			<input type="hidden" name="orgId" value="${supInfo.orgId}"/>
			...
			<table cellpadding="0" cellspacing="0" class="viewTable">
							...
							<option value="">---请选择----</option>
								<c:forEach var="row" items="${cp:DICTIONARY('Charge_Flag')}">
									<option value="${row.key}"
										<c:if test="${object.isExternal eq row.key}"> selected="selected" </c:if>>
				...
				<tr height="22">
					...
					<td><s:textarea name="superviseResult" id="superviseResult"/></td>
				</tr>
				<tr><td colspan="2" align="center">	<input type="button" class="btn" value="督办结论" onclick="save();"/>
	...
	function save(){
		var superviseResult=document.getElementById("superviseResult").value;
		if(""==superviseResult){
			alert("督办意见不能为空");
			document.forms[0].superviseResult.focus();
			return;
		}
		  var form=document.getElementById("supInfoBasicDlfxForm");
		     form.action="supInfoBasicDlfx!saveSuperviseJl.do";     
		     form.submit();
	}
</script>
</html>
