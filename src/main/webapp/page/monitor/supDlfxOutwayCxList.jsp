		...
		<fieldset
			...
			<s:form action="supInfoBasicDlfx" namespace="/supervise" style="margin-top:0;margin-bottom:5" id="supInfoBasicDlfxForm" method="post" >
				<table cellpadding="0" cellspacing="0" class="viewTable">
					...
					<input type="hidden" id="s_orgId" name="s_orgId" value="${param.orgId}"/>
				...
				月：
					...
					<s:submit method="cxList" key="opt.btn.query" cssClass="btn" ></s:submit></td>
					</tr>
				</table>
			</s:form>
		</fieldset>
	<ec:table action="supervise/supInfoBasicDlfx!fkList.do" items="basicList" var="info" imagePath="${pageContext.request.contextPath}/themes/css/images/table/*.gif"  retrieveRowsCallback="limit">
			<ec:row>
				<ec:column property="orgId" title="被督办部门名称" style="text-align:center" sortable="false">
					<a href="${pageContext.request.contextPath}/supervise/supInfoBasicDlfx!supDlfxCx.do??s_superviseCode=${info.superviseCode}&s_orgId=${info.orgId}&s_supervisionMonth=${info.supervisionMonth}&s_supervisionYear=${info.supervisionYear}&s_no=${info.no}">${cp:MAPVALUE("depno",info.orgId)}</a>
				</ec:column>
