...
<body>
		<fieldset>
			...
			<s:form id="dispatchdocForm" action="dispatchDoc!incomeDocList.do" namespace="/dispatchdoc" method="post">
				<input type="hidden" id="dispatchNo" name="dispatchNo" value="${dispatchNo}" />
...
// 	$('table tbody tr:last a').each(function() {
...
// 	});
	...
	jQuery(document).ready(function($) {
		...
		function rowSelected(oCheckBox, index, isClick) { // 选中
			...
			$(oTr).attr("style", "background-color: yellow"); // 设置颜色
			
// 			if (isClick) {
// 				var oTrClone = $(oTr).clone();
// 				$(oTrClone).find("td").eq(0).remove();
// 				$(oTrClone).appendTo($("#ec_table > tbody", window.opener.document));
// 			}
		}
		...
		$("input:checkbox[name='djId']").click(function() {
			...
			if ("checked" == $(this).attr("checked")) { // 选中
		...
		$("#btn_save").click(function() {
			...
			$.ajax({
				type : "POST",
				url : "<%=request.getContextPath()%>/dispatchdoc/dispatchDoc!docRelativeSave.do?dispatchNo=" + $("#dispatchNo").val() + "&incomeNo=" + array.join(","),
				dataType : "json",
