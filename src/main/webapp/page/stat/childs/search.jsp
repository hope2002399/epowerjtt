...
<script>																		  
	$('#unittree-table').treetable({ expandable: true });
			...
			<c:forEach var="cond" items="${conditions}" varStatus="status">
				<c:choose>
					...
					<c:when test="${cond.condStyle =='D'}">
						<li>
							<label>${cond.condLabel}：</label> 
							<input type="text" name="${cond.condName}" class="date" value="${cond.condValue}"
								format="yyyy-MM-dd" yearstart="-10" yearend="0" readonly="true" />
						</li>
					</c:when>
