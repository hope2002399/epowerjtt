...
TToolBox.prototype.hideBox = function()
{
	...
	oImg.src =path+ "/page/workflow/image/" + this.TitleBtn[0] + ".gif";
	oImg.title = "单击展开工具栏";
...
TToolBox.prototype.showBox = function()
{
	...
	oImg.src = path+ "/page/workflow/image/" + this.TitleBtn[1] + ".gif";
...

{
	...
	if(this.Dir == "down")
	{
		...
		'ondblclick="'+this.ID+'.showHideTool();"><table width="100%" height="100%" cellspacing="0" cellpadding="0" border="0"><tr><td width="99%" moveType="m">&nbsp;<font color="#FFFFFF" moveType="m">' + this.Caption + '</font></td><td width="1%"><img id="'+this.ID+'_img" title="单击收缩工具栏" src="'+path+'/page/workflow/image/up.gif" style="cursor:hand;" onclick="'+this.ID+'.showHideTool();" height="14" width="14" border="0"></td></tr></table></td></tr>' +
...
TToolPanel.prototype.toString = function()
{
	...
	this.Width + 'px;height:' + this.Height + 'px;z-index:3000" cellspacing="0" cellpadding="0" border="0" onmousedown="'+this.ID+'.mouseDown();" '+
	'onmousemove="'+this.ID+'.mouseMove();" onmouseup="'+this.ID+'.mouseUp();">' +
	'<tr><td width="8" style="cursor:move;" moveType="m"><img moveType="m" src="'+path+'/page/workflow/image/tb_title.gif" height="25" width="5" border="0"></td><td>' + this.Content + '</td</tr>' +
	'</table>';
	return S;
};
