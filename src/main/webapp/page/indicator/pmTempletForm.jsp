...
<html>
	<head>
		...
		$(document).ready(function(){
			$.ajax({
				   ...
				   data:{
					   ...
					   "object.releaseFlag":$("#releaseFlag").val()
				   },
				   success: function(data){
					   ...
					   if(allIndicators!=null&&allIndicators.length>0){
						   for(var i=0;i<allIndicators.length;i++){
							   ...
							   if(!allIndicators[i].hasOwnProperty("ifHidden")){
								   allIndicators[i].ifHidden="F";
							   }
							   //else{
					   ...
					   if(nowIndicators!=null&&nowIndicators.length>0){
						   for(var i=0;i<nowIndicators.length;i++){
							   ...
							   //alert("123"+!allIndicators[i].hasOwnProperty("ifHidden")||allIndicators[i].ifHidden==null||allIndicators[i].ifHidden=="");
							   //if(!allIndicators[i].hasOwnProperty("ifHidden")){
								//   allIndicators[i].ifHidden="F";
							   //}
							   //else{
		...
		function init() {
			...
			var html='<select class="discript1" name="templetType" id="templetType"><option value="">请选择</option>';
			$.ajax({
				   type : 'get',
		...
		function templetSave(){
			...
			$.ajax({
				   ...
				   success: function(data){
					   //alert(data.flag);
					   //window.open("${pageContext.request.contextPath}/indicator/pmTemplet!compose.do?jspFileName="
						//	   	+ data.jspFileName + "&formName=" + data.formName);
					   window.open("${pageContext.request.contextPath}/indicator/pmTemplet!compose.do?templetId="+data.templetId);
				   }
			});
		}
	</SCRIPT>
		...
		<fieldset style=" display: block; padding: 10px;height: 20%; " >
			...
			<table border="0" cellpadding="0" cellspacing="0" class="viewTable">
				<tr>
					...
					<%-- <td class="tdRight">模版类型:</td>
					<td class="tdRight1">
							<select class="discript1" name="templetType" id="templetType">
								<option value="">请选择</option>
								<c:forEach var="types" items="${typelist}">
									<option type="radio"  value="${types.itemId }" >
										${types.itemName}
									</option>
								</c:forEach>
							</select>
