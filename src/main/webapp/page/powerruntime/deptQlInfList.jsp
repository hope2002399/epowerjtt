					...
					<table width="100%" border="0" cellpadding="0" id="treeTable" class="tableRegion" cellspacing="0" class="zn2">
						<thead align="center">
							<tr>
								...
								<td width="25%" class="tableHeader" >业务部门</td>
							</tr>
						</thead>
						<tbody align="center" class="tableBody" id="tabledy">
							...
							<c:if test="${!(empty dqi.deptQlParentId)}">
								...
								<c:if test="${not empty dqi.orgname1 }">
									<td id="tr4${status.index }" nowrap="nowrap" style="border-bottom: #9d9d9d 1px dotted">${dqi.orgname1 }</td>
								</c:if>
								<c:if test="${empty dqi.orgname1 }">
									<td id="tr4${status.index }" nowrap="nowrap" style="border-bottom: #9d9d9d 1px dotted">${dqi.orgname2 }</td>
								</c:if>
								</tr>
							</c:forEach>
						</tbody>
						...
						for(var i=0;i<=data.deptQlInfs.length;i++){
							if(data.deptQlInfs[i] != undefined){
								if(i%2 ==0){
									...
									s+="	<td id='tr3"+i+"' nowrap='nowrap' style='border-bottom: #9d9d9d 1px dotted'>"+data.deptQlInfs[i].orgshortname+"</td>";
									if(data.deptQlInfs[i].orgname1 != ''){
										s+="	<td id='tr3"+i+"' nowrap='nowrap' style='border-bottom: #9d9d9d 1px dotted'>"+data.deptQlInfs[i].orgname1+"</td></tr>";
									}else{
										s+="	<td id='tr3"+i+"' nowrap='nowrap' style='border-bottom: #9d9d9d 1px dotted'>"+data.deptQlInfs[i].orgname2+"</td></tr>";
									}
								}else{
									if(data.deptQlInfs[i].deptQlParentId == ''||data.deptQlInfs[i].deptQlParentId==null){
										if(data.deptQlInfs[i].zxs == ''||data.deptQlInfs[i].deptQlParentId==null){
