		...
		<fieldset style="display: block; padding: 10px;">
			...
			<table border="0" cellspacing="0" cellpadding="0" class="viewTable"
				...
				<tbody class="tableBody">
					<c:forEach items="${VOpt}" var="optStuff" varStatus="status">
						...
						<td>
						<c:if test="${optStuff.iszhi=='1'}">
						<a style="text-decoration: none; color: blue;"
										href="javascript:void(0);" onclick="alert('纸质文件');">${optStuff.filename}</a>
					
						</c:if>
						<c:if test="${optStuff.iszhi != '1'}">
						<a style="text-decoration: none; color: blue;"
										href="<%=request.getContextPath()%>/powerruntime/generalOperator!downStuffInfo.do?stuffid=${optStuff.stuffid}&netid=${optStuff.netId}">${optStuff.filename}</a>
						</c:if>
							</td>
						</tr>
					</c:forEach>
