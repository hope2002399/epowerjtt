	...
	<s:form action="generalOperator!saveStuff.do" method="POST"
		...
		<table border="0" class="viewTable">
			...
			<c:if test="${ empty suppowerstuffinfos }">
				<tr>
					<td align="center" colspan=5>不需上传材料</td>
				</tr>
			</c:if>
			<!-- ×××××××××××××××××××新的××××××××××××××××××××××××××××× -->
									...
									<c:if test="${info.sortId == obj.sortId}">
										...
										<c:if test="${info.iszhi!= '1'}">
											...
											<br> <script type="text/javascript">
												var sid = '${obj.sortId}';
												$(".${obj.sortId}").attr("disabled", "disabled");
											</script>
										</c:if>
									</c:if>
								</c:forEach>
							</c:if></td>
						<td width="50px" id="stuff_${obj.sortId}"><div
								id="div_${obj.sortId}">
								<input type="file" id="file_${obj.sortId}" name="stuffFile"
...
<script type="text/javascript">
    ...
    function fileChangeStruts(sortId){
   	 var filehtml ="<input type=\"file\" id=\"file_"+sortId+"\" name=\"stuffFile\" style=\"cursor:pointer;title="+Math.floor(Math.random()*10+1)+"\">";
   	 alert(filehtml);
   	 $("#file_"+sortId).replaceWith(filehtml);
   }
