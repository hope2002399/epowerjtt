...
.addTd {
	...
	<div class="search stat">
		<div class="crumbs">电子证照统计（省市）</div>
			<form action="./sys/jttWssbtjsx!zzxxByDepPro.do" method="get" >
			<%
			...
			<span class="addTd">
				 ...
				 value="<%=startTime%>" name="s_startTime" onclick="WdatePicker({dateFmt:'yyyy-MM-dd'})" placeholder="选择日期">
			</span>
			<span class="addTd">
				 <label>结束时间：</label>
...
<div>
	<div>
					<table width="100%" border="0" cellpadding="0" id="treeTable" class="tableRegion" cellspacing="0" class="zn2">
						<caption style="font-size: 16px;padding: 5px;color: #2D80D2;font-weight: bold;">电子证照统计（省市）</caption>
						<thead align="center">
							<tr>  
						        <th scope="col" >部门</th>  
						        <th scope="col" >公路</th>
							 ...
							 List<Map<String,Object>> list = (List<Map<String,Object>>) request.getAttribute("map");
							   ...
							   %>   
									...
									{%>data-tt-parent-id="<%=info.get("parentNo") %>"<%}%>>
										...
										<td ><%=info.get("JS") %></td> 
										<td ><%=info.get("GG") %></td>
										<td ><%=Integer.parseInt(info.get("GL").toString()) + 
										Integer.parseInt(info.get("GG").toString()) +
										Integer.parseInt(info.get("YG").toString()) +
										Integer.parseInt(info.get("GK").toString()) +
										Integer.parseInt(info.get("HS").toString()) +
										Integer.parseInt(info.get("HD").toString()) +
										Integer.parseInt(info.get("ZJ").toString()) +
										Integer.parseInt(info.get("JS").toString())%></td>
									</tr>
									<%}else{%>
...
<script language="JavaScript"
	...
	function exportExcel(){
		window.location.href = '${pageContext.request.contextPath}/sys/exportZzxxPro.do?decisionBeginTime='+$("#decisionBeginTime").val()+"&decisionEndTime="+$("#decisionEndTime").val();
	}
	function openXK(){
		var iWidth=600; //弹出窗口的宽度;
		var iHeight=400; //弹出窗口的高度;
