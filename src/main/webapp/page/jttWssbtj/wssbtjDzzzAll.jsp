...
.addTd {
	...
	<div class="search stat">
		<div class="crumbs">办件信息报送情况</div>
			<form action="。/sys/jttWssbtjsx!dzzzByDepAll.do" method="get" >
			<%
			...
			<div class="addbutton">
				...
				<input type="button" class="btn" value="导出word" onclick="return exportWord();">
			</div>
		</form> 
	</div>
<div>
	<div>
					<table width="100%" border="0" cellpadding="0" id="treeTable" class="tableRegion" cellspacing="0" class="zn2">
						<caption style="font-size: 16px;padding: 5px;color: #2D80D2;font-weight: bold;">办件信息报送情况</caption>
						<thead align="center">
							<tr>  
						        <th scope="col" >部门</th>  
						        <th scope="col" >公路</th>
							 ...
							 List<Map<String,Object>> list = (List<Map<String,Object>>) request.getAttribute("map");
							   ...
							   %>   
									...
									{%>data-tt-parent-id="<%=info.get("parentNo") %>"<%}%>>
										...
										+Integer.parseInt(info.get("JS").toString())+Integer.parseInt(info.get("GG").toString())%></td>
									</tr>
									<%}else{%>
...
<script language="JavaScript"
	...
	function exportExcel(){
		window.location.href = '${pageContext.request.contextPath}/sys/exportBjxxAll.do?decisionBeginTime='+$("#decisionBeginTime").val()+"&decisionEndTime="+$("#decisionEndTime").val();
	}
	function exportWord(){
		window.location.href = '${pageContext.request.contextPath}/sys/jttWssbtjsx!docWriter.do?decisionBeginTime='+$("#decisionBeginTime").val()+"&decisionEndTime="+$("#decisionEndTime").val();
	}
	function openXK(){
		var iWidth=600; //弹出窗口的宽度;
		var iHeight=400; //弹出窗口的高度;
