...
.addTd {
	...
	<div class="search stat">
		<div class="crumbs">业务事项办理情况</div>
			<form action="。/sys/jttWssbtjsx!showywInfHadCount.do" method="get" >
			<%
...
<div>
	<div>
					<table width="100%" border="0" cellpadding="0" id="treeTable" class="tableRegion" cellspacing="0" class="zn2">
						<caption style="font-size: 16px;padding: 5px;color: #2D80D2;font-weight: bold;">业务办理事项统计</caption>
						<thead align="center">
							<tr>  
						        <th scope="col" >业务编码</th>  
						        <th scope="col" >业务名称</th>
						        <th scope="col" >省级办件数量</th>  
						        <th scope="col" >市级办件数量</th>
						        <th scope="col" >区县办件数量</th>  
						        <th scope="col" >合计</th>  
				 		   </tr> 
						</thead>
						<tbody align="center" class="tableBody" id="tabledy">
							 ...
							 List<Map<String,Object>> list = (List<Map<String,Object>>) request.getAttribute("map");
							   ...
							   %>   
									<tr>
										<td ><%=info.get("deptregno") %></td>
										<td ><%=info.get("ywName") %></td> 
										<td ><%=info.get("sh") %></td>
										<td ><%=info.get("city") %></td>
										<td ><%=info.get("qu") %></td> 
										<td ><%=info.get("heji") %></td> 
...
<script language="JavaScript"
	...
	function exportExcel(){
		window.location.href = '${pageContext.request.contextPath}/sys/exportYwHand.do?decisionBeginTime='+$("#decisionBeginTime").val()+"&decisionEndTime="+$("#decisionEndTime").val();
	}
	function openXK(){
		var iWidth=600; //弹出窗口的宽度;
		var iHeight=400; //弹出窗口的高度;
