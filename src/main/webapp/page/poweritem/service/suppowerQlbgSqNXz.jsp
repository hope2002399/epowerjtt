...
<body
	onload="inital('${bi.itemType}');checkReceive(${object.ischarge});">
					...
					<td align="left"><select name="orgId" id="orgId">
							...
							<c:forEach var="row" items="${unitList}">
								<option value="<c:out value='${row.depno}'/>"
									<c:if test="${row.depno==bi.orgId}">selected="selected"</c:if>>
						...
						onchange="checkItemType();" id="itemType">
							...
							<c:forEach var="row" items="${cp:DICTIONARY('ITEM_TYPE_NEW')}">
								<option value="${row.key}"
									<c:if test="${bi.itemType eq row.key}"> selected = "selected" </c:if>>
	...
	/* 	function deleteFile(id, fileType) {
		...
		function checkitemid(){
			...
			$.ajax({
				type:"POST",
				url: "<%=request.getContextPath()%>/poweritem/bpowerItemService!checkItemId.do?type=${type}&itemId="+itemId,
