...
<script type="text/javascript">
	...
	function selectTemplate() {
		...
		parentDocument.getElementById('unitcode').value = tempIds.substring(0,
				tempIds.length - 1);
		parentDocument.getElementById('uninName').value = tempNames.substring(
				0, tempNames.length - 1);
	}
</script>
...
<body>
	<ec:tree identifier="unitcode" parentAttribute="parentunit"
		items="listSelectOrgList"
		action="powerruntime/powerOrgInfo!listSelectOrg.do"
		view="org.extremecomponents.tree.TreeView" filterable="false"
		sortable="false" var="unit"
		imagePath="${pageContext.request.contextPath}/themes/css/images/table/*.gif">
		<ec:row>
			<ec:column property="unitname" title="部门名"
				cell="org.extremecomponents.tree.TreeCell" />
			...
			<ec:column property="unitopt" title="操作" sortable="false">
				<input type="checkbox" name="unitcode" id="unitcode${unit.unitcode}"
					onclick="selectTemplate()"
					value="${unit.unitcode}|${unit.unitname}">
			</ec:column>
		</ec:row>
