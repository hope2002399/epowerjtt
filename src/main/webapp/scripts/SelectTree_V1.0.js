var getID = function( id ){
	...
	Menu.menuDate = {
		...
		sortList:function(list){
			var s = new Array();
			for( var i=0,len=list.length;i<len;i++ ){
				if(s[list[i]["ParentID"]]==undefined)s[list[i]["ParentID"]]=new Array();
		...
		var mv = Menu.menuView,md = Menu.menuDate;
			...
			if(menuList.length){
				var c = mv.headList().join(""),
					...
					c = new Array,f=new Array,newLine=new Array,
							...
							for(var j=0,jLen=sortList[levelKey[h]].length;j<jLen;j++){
								...
								while(m>=0){
									if(pid[p]){
										var oldP=p;
										p=pid[p];
										if( sortList[p] && sortList[p][sortList[p].length-1]["MID"]==oldP ){
											newLine[m]=indent;
										}else{
											newLine[m]=line;
										}
								...
								if(c[levelKey[h]]==undefined) {
									...
									c[levelKey[h]].push("<ul id='ul_"+levelKey[h]+"' class='tree'>");
								}
								newLine.pop();
								if(sortList[sortList[levelKey[h]][j]["MID"]]==undefined){
									newLine.push(node);
								}else if(j==jLen-1&&sortList[sortList[levelKey[h]][j]["MID"]]){
									newLine.push(last_expandable);
								}else{
									newLine.push(expandable);
								}
								c[levelKey[h]].push("<li id='menu_"+sortList[levelKey[h]][j]["MID"]+"' parentId='"+sortList[levelKey[h]][j]["ParentID"]+"'><div>"+newLine.join("")+"<span rel='"+sortList[levelKey[h]][j]["MID"]+"' >"+sortList[levelKey[h]][j]["MText"]+"</span></div></li>");
								if(j==jLen-1)c[levelKey[h]].push("</ul>");
							}
							newLine.length=0;
							f.push(levelKey[h]);
						}
