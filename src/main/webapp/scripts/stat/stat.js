...
var DEPT_INPUT, DEPT_INPUT_CODE, DEPT_INPUT_BOX;
var DEPT_NODES;
// 选择部门
$(function() {
	// 选择部门
	function zTreeOnClick(event, treeId, treeNode) {
		...
		DEPT_INPUT_CODE.val(treeNode.id);
		DEPT_INPUT_BOX.hide();
	};
	
	// 获取部门
	function getDept(callback) {
		$.get("epower-jtt/stat/twodimenform!getUserDeptJSON.do", function(data) {
			DEPT_NODES = data;
			callback && callback.call(this, DEPT_NODES);
		});
	}
	
	// 生成树
	function createTree(data, code) {
		var temp = $.fn.zTree.init(DEPT_INPUT_BOX, {
			data: {
				simpleData: {
					enable: true
				}
		...
		DEPT_INPUT.data('ztree', temp);
		// 展开已选中的节点
		if (code) {
			var node = temp.getNodeByParam("id", code, null);
			node && (node = temp.getNodeByParam("id", node.pId, null));
			temp.expandNode(node, true, false, true, true);
		}
	}
	...
	// 点击展示部门
	$('input.tree-dept').click(function() {
		DEPT_INPUT = $(this);
		$('div.show-dept').not(DEPT_INPUT).hide();
		DEPT_INPUT_CODE = DEPT_INPUT.parent().find('input.tree-dept-code');
		DEPT_INPUT_BOX = DEPT_INPUT.parent().find('div.show-dept').show();
		
		var ztreeObj = DEPT_INPUT.data('ztree');
		if (!ztreeObj) {
			// 展开原来选中节点
			var code = DEPT_INPUT_CODE.val();
			if (!DEPT_NODES) {
				getDept(function(data, code) {
					createTree(data);
				});
			}
			else {
				createTree(DEPT_NODES, code);
			}
		}
	});
	
	// 点击其他隐藏部门
	$(document).click(function(evnet) {
		var target = $(evnet.target);
		
		target.not(DEPT_INPUT).length && target.not(DEPT_INPUT_BOX).length && !target.closest(DEPT_INPUT_BOX).length && DEPT_INPUT_BOX.hide();
	});
});
