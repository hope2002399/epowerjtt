		...
		try{
			if(webObj.WebObject.Tables.Count>0 && rowObj.length > 3 ){ 
				...
				for(var i=2; i< rowObj.length - 1 ; i++){ 
					...
					webObj.WebObject.Tables.Item(1).Cell(i,2).Range.Text=_getRowCellValue(tableObj,i,1);//收文号 
					webObj.WebObject.Tables.Item(1).Cell(i,3).Range.Text=_getRowCellValue(tableObj,i,2);//原文号 
					webObj.WebObject.Tables.Item(1).Cell(i,4).Range.Text=_getRowCellValue(tableObj,i,3);//来文单位 
					webObj.WebObject.Tables.Item(1).Cell(i,5).Range.Text=_getRowCellValue(tableObj,i,4);//文件标题 
					webObj.WebObject.Tables.Item(1).Cell(i,6).Range.Text=_getRowCellValue(tableObj,i,5);//主办部门 
					webObj.WebObject.Tables.Item(1).Cell(i,7).Range.Text=""; 
					webObj.WebObject.Tables.Item(1).Cell(i,8).Range.Text=_getRowCellValue(tableObj,i,6);//阅示主任 
		...
		try{
			if(webObj.WebObject.Tables.Count>0 && rowObj.length > 3){ 
				for(var i=2; i< rowObj.length - 1 ; i++){ 
					...
					webObj.WebObject.Tables.Item(1).Cell(i,5).Range.Text=_getRowCellValue(tableObj,i,4);//签发人（在字号问好步骤完成后，填写最后一个签发的主任） 
					webObj.WebObject.Tables.Item(1).Cell(i,6).Range.Text=_getRowCellValue(tableObj,i,5);//主送单位 
					if(i < rowObj.length - 2){ 
						webObj.WebObject.Tables.Item(1).Rows.Add(); 
					} 
				} 
	...
	function BWDPrint(webObj,rowIndex){
		...
		try{
			if(webObj.WebObject.Bookmarks.Exists("AllContent")){ 
				...
				try{
					webObj.WebObject.Tables.Item(1).Cell(4,4).Range.Text=rowObj.cells[6].innerText;//主办部门
				}catch(e){
					webObj.WebObject.Tables.Item(1).Cell(4,2).Range.Text=rowObj.cells[6].innerText;//主办部门，2003以前版本的引用方式
				}
				webObj.WebObject.Tables.Item(1).Cell(5,2).Range.Text=rowObj.cells[7].innerText;//文件标题 
				webObj.WebObject.Tables.Item(1).Cell(6,2).Range.Text='\n'+rowObj.cells[8].innerText;//批分意见 
				try{
			...
			if(webObj.WebObject.Bookmarks.Exists("AllContent")){ 
				...
				for(var i = 0; i < chks.length; i ++){
					if(chks[i].checked){
						...
						//word2000下对表格中合并单元格之后的坐标引用和2007不太一样
						try{
							webObj.WebObject.Tables.Item(n).Cell(4,4).Range.Text=rowObj.cells[6].innerText;//主办部门
						}catch(e){
							webObj.WebObject.Tables.Item(n).Cell(4,2).Range.Text=rowObj.cells[6].innerText;//主办部门，2003以前版本的引用方式
						}
