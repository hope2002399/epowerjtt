...
public class ExportSdbs extends BaseAction implements ServletResponseAware{
	...
	private void export( String startTime, String endTime, HttpServletRequest request, HttpServletResponse response) throws IOException {
		...
		insertExcel(sheet,this.jtt2XybCfService.getDataListSdbs(startTime, endTime),dataStyle);
		//将以上缓存中的内容写到EXCEL文件中
	...
	private String [] getTitleExcel(){
		String [] title = {"行政机关","成功报送数据量（许可） ","表格上报数据量（许可）","成功报送数据量（处罚）","表格上报数据量（处罚）","合计"};
		return title;
	}
