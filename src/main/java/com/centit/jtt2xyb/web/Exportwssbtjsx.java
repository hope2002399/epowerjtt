...
public class Exportwssbtjsx extends BaseAction implements ServletResponseAware{
	...
	public String execute() throws Exception {
		...
		export(startTime,endTime , request,response);
		return null;
	}
	...
	private void export( String startTime, String endTime, HttpServletRequest request, HttpServletResponse response) throws IOException {
		...
		insertExcel(sheet,this.wssbtjService.getDataListsx(startTime,endTime),dataStyle);
		//将以上缓存中的内容写到EXCEL文件中
	...
	private void getFirstRow(HSSFSheet sheet, String startTime, String endTime, HSSFCellStyle timeStyle) {
		...
		cell.setCellValue("外网申报（市县）统计表("+startTime+"至"+endTime+")");
	...
	private void insertExcel(HSSFSheet sheet, List<Map<String, Object>> dataList, HSSFCellStyle style) {
		...
		for (int i = 0; i < dataList.size(); i++) {
		    if(StringUtils.isNotBlank((String)dataList.get(i).get("parentNo"))){
    			HSSFRow row = sheet.createRow(n + 3);
    			n++;
    			for (int j = 0; j < 10; j++) {
	...
	private void getTitle(HSSFSheet sheet, String[] title, HSSFWorkbook excel, HSSFCellStyle style) {
		...
		String [] title1 = {"部门","申报数量 ","已反馈","未反馈","反馈状态"};
