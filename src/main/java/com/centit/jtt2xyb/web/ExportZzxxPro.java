...
public class ExportZzxxPro extends BaseAction implements ServletResponseAware{
	...
	private void export(HttpServletRequest request, HttpServletResponse response) throws IOException {
		...
		insertExcel(sheet,this.wssbtjService.getDataListZzxxPro(filterMap),dataStyle);
		//将以上缓存中的内容写到EXCEL文件中
	...
	private void getFirstRow(HSSFSheet sheet, String startTime, String endTime, HSSFCellStyle timeStyle) {
		CellRangeAddress region1 = new CellRangeAddress(0, 0, 0, 8);
		sheet.addMergedRegion(region1); 
		HSSFRow row = sheet.createRow(0);
		Cell cell =  row.createCell(0);
		cell.setCellValue("电子证照统计（省市）");
	...
	private void insertExcel(HSSFSheet sheet, List<Map<String, Object>> dataList, HSSFCellStyle style) {
	    ...
	    int n = 0;
		for (int i = 0; i < dataList.size(); i++) {
		    if(StringUtils.isNotBlank((String)dataList.get(i).get("parentNo")) || "合计".equals(dataList.get(i).get("orgname").toString())
                    || "江苏省交通运输厅".equals(dataList.get(i).get("orgname").toString())){
