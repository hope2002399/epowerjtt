...
public class FormDataManagerImpl implements FormDataManager {
     ...
     * 
     * @see
     * com.centit.stat.twodimenform.FormDataManager#getDataModel(java.lang.String
     * )
     */
     ...
     * 
     * @see
     * com.centit.stat.twodimenform.FormDataManager#findDataBySql(java.lang.
     * String)
     */
    ...
    public List<Object[]> findDataBySql(String sql) {
        ...
        return baseDao.findObjectsBySqlQuery(statQuery);
    }

    // /* (non-Javadoc)
    // * @see
    // com.centit.stat.twodimenform.FormDataManager#queryFormData(com.centit.stat.twodimenform.FormDataModel,
    // boolean)
    // */
    // @Override
    ...
    // rowLogicUrl = rowLogicUrl.replaceAll(col.getColName(),
    // objs[i].toString());
    //
    // // 替换列逻辑里的列值
    ...
    @SuppressWarnings("unchecked")
    @Override
    strictfp public Integer queryFormData(FormDataModel formData,
            boolean needSum, boolean isPaging) {
        formData.setFormData(baseDao.findObjectsBySqlQuery(formData
                .makeStatQuery(isPaging)));
        ...
        if (needSum) {
            ...
            for (int di = 0; di < datas.size(); di++) {
                for (int i = 1; i < formData.getDataColumnCount(); i++) {
                    if (datas.get(di)[i] != null
                            && datas.get(di)[i] instanceof java.lang.Number)
                        sumData[i] = (sumData[i] == null) ? datas.get(di)[i]
                                : ((Number) sumData[i]).doubleValue()
                                        + ((Number) datas.get(di)[i])
                                                .doubleValue();
                    ;
                }
            }
        ...
        // 链接参数
        List<Map<String, Object>> params = parseDataMap(datas, columns,
                formData.getConditions());
        ...
        for (Object[] row : datas) {
            ...
            int colIndex = 0;
            for (QueryColumn col : columns) {
                tbody.addCell(CTableCell.createTableCell(row[colIndex++], col,
                        param));
            }
    ...
    private static int compareObjects(Object[] obj1, Object[] obj2,
            int rowGroup, List<QueryColumn> cols) {
        for (int i = 0; i < rowGroup; i++) {
            ...
            if (obj1[i] != null || obj2[i] != null) {
                ...
                // 数值
                if ("N".equals(ct)) {
                    int nc = Double.valueOf(obj1[i].toString()).compareTo(
                            Double.valueOf(obj2[i].toString()));
                    if (nc < 0)
                        return -1;
                    if (nc > 0)
                        return 1;
    ...
    private static void mergeObjects(Object[] destObj, Object[] obj1,
            Object[] obj2, int rowGroup, int colSum) {
        if (obj1 == null) {
            if (obj2 == null)
                return;
            for (int i = 0; i < rowGroup; i++)
    ...
    @SuppressWarnings("unchecked")
    @Override
    strictfp public Integer queryCompareData(FormDataModel formData,
            boolean needSum) {
        // 查询数据
        String currTitle = formData.makeCondCompareValue(
                formData.getModelType(), 0);
        List<Object[]> currDatas = baseDao.findObjectsBySqlQuery(formData
                .makeStatQuery());
        String prevTitle = formData.makeCondCompareValue(
                formData.getModelType(), -1);
        List<Object[]> prevDatas = baseDao.findObjectsBySqlQuery(formData
                .makeStatQuery());
        ...
        while (j < currDc && i < prevDc) {
            ...
            int bc = compareObjects(prevDatas.get(i), currDatas.get(j),
                    rowGroup, cols);
            if (bc < 0) {
                ...
                mergeObjects(rowData, prevDatas.get(i), currDatas.get(j),
                        rowGroup, colSum);
                i++;
                j++;
            }
        ...
        if (needSum) {
            ...
            for (int di = 0; di < compareDatas.size(); di++) {
                for (i = rowGroup; i < dataColCount; i++) {
                    if (compareDatas.get(di)[i] != null
                            && compareDatas.get(di)[i] instanceof java.lang.Number)
                        sumData[i] = (sumData[i] == null) ? compareDatas
                                .get(di)[i] : ((Number) sumData[i])
                                .doubleValue()
                                + ((Number) compareDatas.get(di)[i])
                                        .doubleValue();
                    ;
                }
            }
        ...
        List<Map<String, Object>> dataMap = parseDataMap(compareDatas, cols,
                formData.getConditions());
     ...
     * @return
     */
    private static CTableBodyTBody parseCompareTbody(
            List<Object[]> compareDatas, List<QueryColumn> cols,
        ...
        for (Object[] datas : compareDatas) {
            ...
            for (QueryColumn col : columnHead) {
                CTableCell cell = CTableCell.createTableCell(
                        datas[indexColHead++], col, dataMap.get(indexRow));
            ...
            if ("R".equals("R")) {
                for (QueryColumn col : columnData) {
                    CTableCell cellPrev = CTableCell.createTableCell(
                            datas[indexColData], col, dataMap.get(indexRow));
                    CTableCell cellCurr = CTableCell.createTableCell(
                            datas[indexColData + lengthData], col,
        ...
        for (int i = 0; i < colLength; i++) {
            ...
            for (int j = 0; j < rowLength; j++) {
                ...
                if (o instanceof String) {
                    ...
                    data.add(DatetimeOpt.convertTimeToString((Date) o));
     ...
     * @return
     */
    private static CTableBodyTHead parseCrossThead(List<QueryColumn> columns,
            List<Object[]> dataColumns) {
        CTableBodyTHead thead = new CTableBodyTHead();
        ...
        int rowspan = (columnData.size() > 1) ? (headColumns.size() + 1)
                : headColumns.size();
        ...
        if (dataLength > 1) {
            ...
            for (int i = 0; i < dataColumns.size(); i++) {
                for (QueryColumn col : columnData) {
                    CTableCell cell = CTableCell.createTableHeadCell(
                            col.getColName(), columnColumn.get(index - 1));
                    thead.addCell(cell);
                }
            }
        }
     ...
     * @return
     */
    private static CTableBodyTHead parseCompareThead(List<QueryColumn> cols,
            String currTitle, String prevTitle) {
        CTableBodyTHead thead = new CTableBodyTHead();
        ...
        if ("R".equals("R")) {
            ...
            for (QueryColumn col : columnData) {
                ...
                CTableCell prevCell = CTableCell
                        .createTableHeadCell(prevTitleChild);
                CTableCell currCell = CTableCell
                        .createTableHeadCell(currTitleChild);
        ...
        // 日期在上、数据在下分组
            ...
            if (multiData) {
                ...
                for (int i = 0; i < 2; i++) {
                    for (QueryColumn col : columnData) {
                        CTableCell cell = CTableCell.createTableHeadCell(col
                                .getColName());
                        thead.addCell(cell);
                    }
                }
     ...
     * @return
     */
    private static List<Map<String, Object>> parseDataMap(List<Object[]> datas,
            List<QueryColumn> cols, List<QueryCondition> conditions) {
     ...
     * @return
     */
    private static List<QueryColumn> parseQueryColumn(List<QueryColumn> cols,
            String showType) {
        if (null == showType)
            return cols;
    ...
    private static int foundCrossColumn(List<Object[]> dataColumns,
            Object[] col, int colCount, int colGroup) {
    ...
    private static void initCrossRowData(Object[] rowData, Object[] curData,
            int rowGroup, int dataColCount) {
        for (int i = 0; i < rowGroup; i++) {
            rowData[i] = curData[i];
        }
    ...
    @SuppressWarnings("unchecked")
    @Override
    strictfp public Integer queryCrossData(FormDataModel formData,
            boolean needSum) {
        ...
        if (columnSql != null && !"".equals(columnSql)) {
            List<Object[]> colList = baseDao.findObjectsBySqlQuery(formData
                    .makeStatQuery(columnSql));
            if (colList != null && colList.size() > 0) {
                for (int i = 0; i < colList.size(); i++) {
                    ...
                    int nf = foundCrossColumn(dataColumns, col, dataCols,
                            colGroup);
                    if (nf < 0) {
                        dataColumns.add(col);
                        dataCols++;
                    ...
                    for (int j = 1; j < colGroup; j++)
                        col2[j] = col[j];
                    nf = foundCrossColumn(sumDataColumns, col2, sumDataCols,
                            colGroup);
                    if (nf < 0) {
        ...
        for (int i = 0; i < currDc; i++) {
            ...
            if (nf >= 0) {// 这个肯定应该为true
                for (int j = 0; j < dataAnalyseSum; j++) {
                    rowData[rowGroup + nf * dataAnalyseSum + j] = curData[rowGroup
                            + colGroup + j];
                }
            }
            col[0] = "合计";
            ...
            if (nf > 0) {
                for (int j = 0; j < dataAnalyseSum; j++) {
                    rowData[rowGroup + nf * dataAnalyseSum + j] = ReflectionOpt
                            .addTwoObject(rowData[rowGroup + nf
                                    * dataAnalyseSum + j], curData[rowGroup
                                    + colGroup + j]);
                }

                /*
                 * 不显示小数点及后面的0 add by zdy
                 */
                if (rowData[rowData.length - 1] != null
                        && rowData[rowData.length - 1] instanceof java.lang.Number) {
                    double d = ((Number) rowData[rowData.length - 1])
                            .doubleValue();
                    if (Math.round(d) - d == 0)
        ...
        if (needSum) {
            ...
            for (int di = 0; di < crossDatas.size(); di++) {
                for (int i = rowGroup; i < dataColCount; i++) {
                    if (crossDatas.get(di)[i] != null
                            && crossDatas.get(di)[i] instanceof java.lang.Number) {
                        sumData[i] = (sumData[i] == null) ? crossDatas.get(di)[i]
                                : ((Number) sumData[i]).doubleValue()
                                        + ((Number) crossDatas.get(di)[i])
                                                .doubleValue();
                        // 报错，为什么要转换成String类型？
        ...
        List<Map<String, Object>> dataMap = parseDataMap(crossDatas, columns,
                formData.getConditions());
        CTableBodyTBody tbody = parseCrossTbody(crossDatas, columns, dataMap,
                dataColumns, formData);
    ...
    private static CTableBodyTBody parseCrossTbody(List<Object[]> crossDatas,
        ...
        for (int rowIndex = 0; rowIndex < crossDatas.size(); rowIndex++) {
            ...
            // 固定列
            for (QueryColumn col : columnHead) {
                CTableCell cell = CTableCell.createTableCell(datas[colIndex++],
                        col, params);
                tbody.addCell(cell);
            }
            ...
            for (Object[] obj : dataColumns) {
                ...
                for (int paramIndex = 0; paramIndex < obj.length; paramIndex++) {
                    params.put(":" + columnCross.get(paramIndex).getColName(),
                            obj[paramIndex].toString());
                }

                for (QueryColumn col : columnData) {
                    CTableCell cell = CTableCell.createTableCell(
                            datas[colIndex++], col, params);

                    // 如果有公共行链接，则加上公共行链接头
                    if (!StringUtils.isBlank(rowLogic)
                            && !StringUtils.isBlank(cell.getHref())) {
