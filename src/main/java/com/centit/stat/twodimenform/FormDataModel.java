...
public class FormDataModel {
    ...
    public static String pretreatmentQuerySql(String querySql,
        ...
        for (QueryCondition cond : getConditions()) {
            ...
            if (cond.getCondValue() != null && !"".equals(cond.getCondValue())) {
                ...
                if (condMap.get(cond.getCondPlace()) != null)
                    filterSql = condMap.get(cond.getCondPlace()) + " "
                            + cond.getCondFilterSql();
                else
            ...
            if ("3".equals(cond.getCompareType())) {
                try {
                    ...
                    if (StringRegularOpt.isNumber(cond.getCondValue())) {
                        ...
                        cond.setCondValue(DatetimeOpt
                                .convertDatetimeToString(DatetimeOpt.addYears(
                                        DatetimeOpt.smartPraseDate(cond
                                                .getCondValue()), offset)));
            ...
            // 月比较
                ...
                try {
                    cond.setCondValue(DatetimeOpt.convertDateToString(
                            DatetimeOpt.addMonths(DatetimeOpt
                                    .smartPraseDate(cond.getCondValue()),
                                    offset), "yyyy-MM"));
                } catch (NumberFormatException e) {
     ...
     * 
     * public void setStatQuery(String sql) { this.statQuery = new
     * SQLWithNamedParamsCallBack(sql); }
     * 
    ...
    public JSONObject toJsonData() {
        ...
        // i=0;
        // for(QueryCondition cond:conditions){
        // JSONOpt.setAttribute(objJson, "conditions["+i+"]",
        // cond.toJsonData());
        // i++;
        // }
        // }
        ...
        if (columns != null) {
            i = 0;
            for (QueryColumn col : columns) {
                JSONOpt.setAttribute(objJson, "columns[" + i + "]",
                        col.toJsonData());
                i++;
            }
        }
