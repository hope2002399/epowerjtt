...
public class QueryModel implements java.io.Serializable {
    ...
    private Set<QueryColumn> queryColumns = null;// new
                                                 // ArrayList<QueryColumn>();
    private Set<QueryCondition> queryConditions = null;// new
                                                       // ArrayList<QueryConditon>();
    ...
    /** full constructor */
    public QueryModel(String modelName, String modelType, String ownerType,
            String ownerCode, String querySql, String queryDesc,
            String formNameFormat, String resultName, String rowDrawChart,
            Long drawChartBeginCol, Long drawChartEndCol, String additionRow,
            String rowLogic, Long rowLogicValue, String logicUrl) {
    ...
    public void replaceQueryColumns(List<QueryColumn> queryColumns) {
        ...
        for (QueryColumn newdt : newObjs) {
            found = false;
            for (Iterator<QueryColumn> it = getQueryColumns().iterator(); it
                    .hasNext();) {
                QueryColumn odt = it.next();
                if (odt.getCid().equals(newdt.getCid())) {
                    odt.copy(newdt);
    ...
    public void replaceQueryConditons(List<QueryCondition> queryConditons) {
        ...
        for (QueryCondition newdt : newObjs) {
            found = false;
            for (Iterator<QueryCondition> it = getQueryConditions().iterator(); it
                    .hasNext();) {
                QueryCondition odt = it.next();
                if (odt.getCid().equals(newdt.getCid())) {
                    odt.copy(newdt);
