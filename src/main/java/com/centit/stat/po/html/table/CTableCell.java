...
public class CTableCell extends AbstractCHtmlComponent {
     ...
     */
    public static final Pattern PATTERN_DISPLAY = Pattern
            .compile("display:(.*?);");
     ...
     * HTML模板 带链接
     */
    public static final String TABLE_CELL_MODEL_LINK = "<td ${id}${class}${style}${rowspan}${colspan} data-value='${value}'><a href='${href}'${target} external='true'>${data}</a></td>";
     ...
     * @return
     */
    public static CTableCell createTableCell(Object orignValue,
            QueryColumn col, Map<String, Object> params) {
    ...
    private static boolean parseDisplay(QueryColumn col) {
        // 为空或者值为 T 均显示
        if (null == col || StringUtils.isBlank(col.getIsShow())
                || "T".equals(col.getIsShow())) {
            return true;
        }
    ...
    private static String parseHref(QueryColumn col, Map<String, Object> params) {
        ...
        for (Entry<String, Object> entry : params.entrySet()) {
            ...
            colLogic = colLogic.replaceAll(entry.getKey(),
                    "" + entry.getValue());
        }
    ...
    private static String parseDisplayValue(Object orignValue, QueryColumn col) {
        ...
        if ("D".equals(colType)) {
            ...
            return DatetimeOpt.convertDateToString(
                    DatetimeOpt.smartPraseDate(value), format);
        }
    ...
    public String getHtml() {
        ...
        String strTarget = (StringUtils.isEmpty(linkTarget) ? "" : " target='"
                + linkTarget + "'");
        String strStyle = (StringUtils.isEmpty(getCssStyle()) ? "" : " style='"
                + getCssStyle() + "'");
        String strClass = (StringUtils.isEmpty(getCssClass()) ? "" : " class='"
                + getCssClass() + "'");
        ...
        if (StringUtils.isEmpty(href)) {
            ...
            if (isHead) {
                ...
                html = TABLE_CELL_MODEL_LINK.replace("${href}", href).replace(
                        "${target}", strTarget);
            }
        }
        ...
        return html = html.replace("${id}", strId)
                .replace("${class}", strClass).replace("${style}", strStyle)
                .replace("${rowspan}", strRowspan)
                .replace("${colspan}", strColspan)
                .replace("${data}", displayValue).replace("${value}", strValue);
    }
}
