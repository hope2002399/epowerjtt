...
public class FUserrole implements java.io.Serializable {
    ...
    public void setObtaindateToToday() {
        if (this.id == null)
            this.id = new FUserroleId();
        this.id.setObtaindateToToday();// ( java.util.Date.valueOf( (new
     ...
     * 
     * try { this.id.setObtaindate(sdfDate.parse(sobtaindate)); } catch
    ...
    public String display() {
        ...
        if (null != this.secededate) {
            secededateStr = ", 到期时间="
                    + DatetimeOpt.convertDateToString(this.secededate,
                            "yyyy-MM-dd HH:mm:ss");
        }

        return "[用户信息 [用户名 = "
                + CodeRepositoryUtil.getValue("usercode", id.getUsercode())
                + ",角色名 = "
                + CodeRepositoryUtil.getValue("rolecode", id.getRolecode())
                + ", 获得角色时间 = "
                + DatetimeOpt.convertDateToString(id.getObtaindate(),
                        "yyyy-MM-dd HH:mm:ss") + secededateStr + "]]";
    }
}
