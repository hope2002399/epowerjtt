...
public class OptInfoDao extends BaseDaoImpl<FOptinfo> {
    ...
    public List<FOptinfo> getFunctionsByUserID(String userID) {
        ...
        List<FVUseroptmoudlelist> ls = getHibernateTemplate().find(hql,
                (Object[]) params);
    ...
    public List<FOptinfo> getMenuFuncByUserID(String userID) {
        ...
        // + " ORDER BY preoptid, formcode";
        List<FVUseroptmoudlelist> ls = (List<FVUseroptmoudlelist>) getHibernateTemplate()
                .find(hql, new Object[] { userID });

        return getMenuFuncs(preOpts, ls);
    }

    @SuppressWarnings("unchecked")
    public List<FOptinfo> getMenuFuncByUserIDAndSuperFunctionId(String userID,
        ...
        // + " ORDER BY preoptid, formcode";
        List<FVUseroptmoudlelist> ls = (List<FVUseroptmoudlelist>) super
    ...
    private static List<FOptinfo> getMenuFuncs(List<FOptinfo> preOpts,
            List<FVUseroptmoudlelist> ls) {
        ...
        for (FVUseroptmoudlelist opm : ls) {
            ...
            opts.add(opt);
            for (int i = 0; i < preOpts.size(); i++) {
                if (opt.getPreoptid() != null
                        && opt.getPreoptid().equals(preOpts.get(i).getOptid())) {
                    isNeeds[i] = true;
                    break;
                }
            }
        ...
        while (true) {
            ...
            for (int i = 0; i < needAdd.size(); i++) {
                for (int j = 0; j < preOpts.size(); j++) {
                    if (!isNeeds[j]
                            && needAdd.get(i).getPreoptid() != null
                            && needAdd.get(i).getPreoptid()
                                    .equals(preOpts.get(j).getOptid())) {
                        isNeeds[j] = true;
                        isNeeds2[j] = true;
                        nestedMenu++;
                        break;
    ...
    public List<FOptinfo> findMenuFuncByType(String type) {
        ...
        @SuppressWarnings("unchecked")
        List<FOptinfo> opts = getHibernateTemplate().find(hql,
                (Object[]) params);
    ...
    @SuppressWarnings("unchecked")
    public List<FOptinfo> getFunctionsByUserAndSuperFunctionId(String userID,
            String superFunctionId) {
        String[] params = null;
        String hql = "FROM FVUseroptmoudlelist  where usercode=? and topoptid=?"
                + " ORDER BY preoptid, orderind";
    ...
    public List<FOptdef> getMethodByUserAndOptid(String userCode, String optid) {
        ...
        List<FVUseroptlist> ls = (List<FVUseroptlist>) getHibernateTemplate()
                .find(hql, (Object[]) params);
    ...
    public List<FOptRoleMap> getAllOptRoleMap() {
        ...
        String sSqlsen = "select c.opturl,b.optmethod,a.rolecode,c.optid,b.optcode "
                ...
                + "where c.OptType<>'W' and c.opturl<>'...' "
                + "order by c.opturl,b.optmethod,a.rolecode";
        List<FOptRoleMap> l = (List<FOptRoleMap>) findObjectsBySql(sSqlsen,
                FOptRoleMap.class);
        return l;
    }
    ...
    public List<FOptinfo> getParentMenu() {
        ...
        List<FOptinfo> parentMenuList = (List<FOptinfo>) getHibernateTemplate()
                .find(hql1);
    ...
    public List<FVUseroptmoudlelist> getMenuItem(String userID) {
        ...
        return (List<FVUseroptmoudlelist>) getHibernateTemplate().find(hql,
                (Object[]) params);
    }
