...
public class UserUnitDao extends BaseDaoImpl<FUserunit> {
    ...
    public FUserunit getUserunitByUserid(String userid) {
        @SuppressWarnings("unchecked")
        List<FUserunit> list = getHibernateTemplate()
                .find("FROM FUserunit fUserunit where fUserunit.isprimary = 'T' and fUserunit.id.usercode=? ",
                        userid);
        if (list != null) {
            return list.get(0);
        } else {
    ...
    public List<FUserunit> getSysUsersByUnitId(String unitCode,
            PageDesc pageDesc, Map<String, Object> filterMap) {
    ...
    public List<FUserunit> listUserunitByIds(List<FUserunitId> ids) {
        return super.getHibernateTemplate().findByCriteria(
                DetachedCriteria.forClass(FUserunit.class).add(
