...
public class OptDefDao extends BaseDaoImpl<FOptdef> {
    ...
    public List<FOptdef> getOptDefByOptID(String sOptID) {
        return getHibernateTemplate().find("FROM FOptdef WHERE optid =?",
                sOptID);
    }

    public int getOptDefSumByOptID(String sOptID) {
        return Integer.valueOf(getHibernateTemplate()
                .find("SELECT count(optcode) FROM FOptdef WHERE optid = ?",
                        sOptID).get(0).toString());
    }

    @SuppressWarnings("unchecked")
    public List<VOptDef> getVOptDefs(String wfcode) {
        return getHibernateTemplate().find("FROM VOptDef WHERE wfcode= ?",
                wfcode);
    }
