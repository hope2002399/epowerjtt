...
public class DeptManagerAction extends BaseEntityExtremeAction<FUnitinfo> {
    ...
    public String editDeptPower() {
        try {
            ...
            List<FRolepower> list = sysRoleManager.getRolePowers(sParentUnit
                    + "$" + object.getUnitcode());
    ...
    public String saveDeptPower() {
        try {
            ...
            try {
                sysRoleManager.saveRolePowers(
                        sParentUnit + "$" + object.getUnitcode(), optcodelist);
                savedMessage();

                SYS_OPT_LOG.log(
                        ((FUserinfo) this.getLoginUser()).getUsercode(),
                        "DEPTPOW", String.valueOf(object.getUnitcode()),
                        "saveDeptPower", object.display(), null);
            } catch (Exception e) {
    ...
    public String editDeptRole() {
        ...
        try {
            ...
            if (roleinfo != null) {
                FRoleinfo dbobject = sysRoleManager.getObjectById(roleinfo
                        .getRolecode());
                ...
                List<FRolepower> list = sysRoleManager.getRolePowers(roleinfo
                        .getRolecode());
                powerlist = new HashMap<String, String>();
                for (FRolepower p : list) {
    ...
    public String builtDeptRole() {
        try {
            ...
            if (dept != null) {
                ...
                // List<FRolepower> list =
                // sysRoleManager.getRolePowers(roleinfo.getRolecode());
                powerlist = new HashMap<String, String>();
                // for(FRolepower p:list){
                // powerlist.put(p.getOptcode(), "1");//p.getRolecode());
                // }
                fOptPowers = sysRoleManager
                        .getOptWithPowerUnderUnit(sParentUnit);
            }
            return "editDeptRole";
        } catch (Exception e) {
            e.printStackTrace();
        ...
        try {
            ...
            try {
                if (!roleinfo.getRolecode().startsWith(sParentUnit + "-")) {
                    ...
                    roleinfo.setRolecode(sParentUnit + "-"
                            + roleinfo.getRolecode());
                }
                FRoleinfo dbRoleinfo = sysRoleManager.getObjectById(roleinfo
                        .getRolecode());
                ...
                if (dbRoleinfo != null) {
                    ...
                    sysRoleManager.saveRolePowers(dbRoleinfo.getRolecode(),
                            optlist);
                    // roleinfo = dbRoleinfo;
                } else {
                    ...
                    sysRoleManager.saveRolePowers(roleinfo.getRolecode(),
                            optlist);
                    // roleinfo = new FRoleinfo();
                }
                // 刷新系统中的缓存
                ...
                SYS_OPT_LOG.log(
                        ((FUserinfo) this.getLoginUser()).getUsercode(),
                        roleinfo.getRolecode(), roleinfo.display(),
                        dbRoleinfo == null ? "" : dbRoleinfo.display());
            } catch (Exception e) {
                log.error(e.getMessage(), e);
                saveError(e.getMessage());
                return ERROR;
    ...
    public String deleteDeptRole() {
        try {
            ...
            try {
                FRoleinfo dbObject = sysRoleManager.getObjectById(roleinfo
                        .getRolecode());
                sysRoleManager.deleteObject(dbObject);
                deletedMessage();
                // 刷新系统中的缓存
                codeRepositoryManager.refresh("rolecode");
    ...
    public String listuser() {
        try {
            Limit limit = ExtremeTableUtils.getLimit(request);
            PageDesc pageDesc = ExtremeTableUtils.makePageDesc(limit);
            Map<Object, Object> paramMap = (Map<Object, Object>) request
                    .getParameterMap();
            ...
            userList = (userList == null ? new ArrayList<FUserinfo>()
                    : userList);
    ...
    public String viewUserRole() {
        ...
        try {
            ...
            FUserinfo dbobject = sysUserManager.getObjectById(userinfo
                    .getUsercode());
            if (dbobject == null) {
            ...
            if (dept != null) {
                String sParentUnit = dept.getUnitcode();
                userroles = sysUserManager.getAllUserRoles(
                        userinfo.getUsercode(), sParentUnit + "-");
                List<FUserrole> list = sysUserManager.getAllUserRoles(
                        userinfo.getUsercode(), "P-");
                if (list != null)
    ...
    public String editUserRole() {
        try {
            ...
            userrole = sysUserManager.getValidUserrole(id.getUsercode(),
                    id.getRolecode());
            FUserDetail user = ((FUserDetail) getLoginUser());
            FUserunit dept = sysUserManager.getUserPrimaryUnit(user
                    .getUsercode());
    ...
    public String saveUserRole() {
        try {
            ...
            FUserrole desobj = sysUserManager.getValidUserrole(
