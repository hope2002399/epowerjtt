...
public class CaptchaImageUtil {
     ...
     * DefaultKaptcha producer = new DefaultKaptcha(); producer.setConfig(new
     * Config(new Properties())); BufferedImage image =
     * producer.createImage(captchaKey); return image;
     * 
     * }
     */
    ...
    public static BufferedImage generateCaptchaImage(String captchaKey) {
        ...
        for (int i = 0; i < captchaKey.length(); i++) {
            g.drawString(captchaKey.substring(i, i + 1),
                    13 * i + 6 + rand.nextInt(5), 14 + rand.nextInt(6));
        }
        // 图象生效
        g.dispose();
        return image;
