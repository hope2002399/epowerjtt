...
public class SysUserFilterEngine implements Serializable {
    ...
    private static class RoleFilterGene {
        ...
        public boolean matchRank(int nR) {
            return rankPlus ? (nR >= xzRank) : (rankMinus ? nR <= xzRank
                    : nR == xzRank);
        }

    }// end of class RoleFilterGene

    public static Set<String> getUsersByRoleAndUnit(String roleType,
            String roleCode, String unitCode) {
        List<FUserunit> lsUserunit = new LinkedList<FUserunit>();
        ...
        if ("gw".equals(roleType)) {
            for (Iterator<FUserunit> it = lsUserunit.iterator(); it.hasNext();) {
                FUserunit uu = it.next();
                // 过滤掉不符合要求的岗位
    ...
    private static Set<String> getUsersByFilter(RoleFilterGene rf) {
        boolean hasFilter = rf.isHasGWFilter() || rf.isHasRankFilter()
                || rf.isHasUnitFilter() || rf.isHasUserFilter()
                || rf.isHasXZFilter();
        ...
        if (rf.isHasRankFilter()) {
            if (rf.isRankAllSub() || rf.isRankAllTop()) { // 所有下级
                for (Iterator<FUserunit> it = lsUserunit.iterator(); it
                        .hasNext();) {
                    FUserunit uu = it.next();
                    if (!rf.matchRank(uu.getXzRank()))
                        it.remove();
                }
            } else {
                ...
                for (FUserunit uu : lsUserunit) {
                    if (rf.matchRank(uu.getXzRank())) {
                        ...
                        if (nR == null) {
                            unitRank.put(uu.getUnitcode(), uu.getXzRank());
                        } else {
                            if (rf.isRankPlus()
                                    && nR.intValue() > uu.getXzRank())
                                unitRank.put(uu.getUnitcode(), uu.getXzRank());
                            else if (rf.isRankMinus()
                                    && nR.intValue() < uu.getXzRank())
                                unitRank.put(uu.getUnitcode(), uu.getXzRank());
                        }
                    }
                ...
                for (Iterator<FUserunit> it = lsUserunit.iterator(); it
                    ...
                    // 过滤掉不符合要求的职位
                    Integer nR = unitRank.get(uu.getUnitcode());
                    if (nR == null || nR.intValue() != uu.getXzRank())
                        it.remove();
                }
            }
    ...
    private static boolean calcUnits(ExpCalcContext ecc, RoleFilterGene gene) {
        ...
        if (!"(".equals(w)) { // 语法错误
            ecc.setLastErrMsg(w
                    + " is unexpected, expect '(' ; calcRoleUnits begin .");
            return false;
        }
    ...
    private static boolean calcUsers(ExpCalcContext ecc, RoleFilterGene gene) {
        ...
        if (!"(".equals(w)) { // 语法错误
            ecc.setLastErrMsg(w
                    + " is unexpected, expect '(' ; calcRoleUsers begin .");
            return false;
        }
        while (true) {
            w = ecc.getAWord();
            ...
            if (")".equals(w)) {
                ...
                ecc.setLastErrMsg(w
                        + " is unexpected, expect ')' or ',' ; calcRoleUsers end .");
                return false;
            }
        }
    }
    ...
    private static boolean calcGwRoles(ExpCalcContext ecc, RoleFilterGene gene) {
        ...
        if (!"(".equals(w)) { // 语法错误
            ecc.setLastErrMsg(w
                    + " is unexpected, expect '(' ; calcGwRoles begin .");
            return false;
        }
        while (true) {
            w = ecc.getAWord();
            if (w == null || "".equals(w)) {
                ecc.setLastErrMsg(w
                        + " is unexpected, expect ')' ; calcGwRoles end .");
                return false;
            }
            ...
            if (ExpCalcContext.isLabel(w)) { // 变量
                ...
                ecc.setLastErrMsg(w
                        + " is unexpected, expect label or string [rolecode]; calcGwRoles label . ");
                return false;
            }
            ...
            if (")".equals(w)) {
                ...
                ecc.setLastErrMsg(w
                        + " is unexpected, expect ')' or ','  ; calcGwRoles , .");
                return false;
            }
        }
    }
    ...
    private static boolean calcXzRoles(ExpCalcContext ecc, RoleFilterGene gene) {
        ...
        if (!"(".equals(w)) { // 语法错误
            ecc.setLastErrMsg(w
                    + " is unexpected, expect '(' ; calcXzRoles begin . ");
            return false;
        }
        while (true) {
            w = ecc.getAWord();
            if (w == null || "".equals(w)) {
                ecc.setLastErrMsg(w
                        + " is unexpected, expect ')'  ; calcXzRoles end .");
                return false;
            }
            ...
            if (ExpCalcContext.isLabel(w)) { // 变量
                ...
                ecc.setLastErrMsg(w
                        + " is unexpected, expect label or string [rolecode]; calcXzRoles label . ");
                return false;
            }
            ...
            if (")".equals(w)) {
                ...
                ecc.setLastErrMsg(w
                        + " is unexpected, expect ')' or ',' ; calcXzRoles , .");
                return false;
            }
        }
    }
    ...
    private static boolean calcXzRank(ExpCalcContext ecc, RoleFilterGene gene) {
        ...
        if (!"(".equals(w)) { // 语法错误
            ecc.setLastErrMsg(w
                    + " is unexpected, expect '(' ; calcXzRank begin . ");
            return false;
        }
        ...
        if (ExpCalcContext.isLabel(w)) { // 变量
            ...
            ecc.setLastErrMsg(w
                    + " is unexpected, expect label or number [rolerank]; calcXzRank label . ");
            return false;
        }
        w = ecc.getAWord();
        if ("+".equals(w)) {
            ...
            if (!StringRegularOpt.isNumber(w)) {
                if (!")".equals(w)) {
                    ecc.setLastErrMsg(w
                            + " is unexpected, expect number or ')' ; calcXzRank +  .");
                    return false;
                } else
                    ecc.setPreword(w);
            } else {
        ...
        if (")".equals(w)) {
            ...
            ecc.setLastErrMsg(w
                    + " is unexpected, expect ')' ; calcXzRank end  .");
            return false;
        }
    }
    ...
    private static Set<String> calcSimpleExp(ExpCalcContext ecc) {
        ...
        while (true) {
            ...
            if ("D".equalsIgnoreCase(w)) {
                ...
                ecc.setLastErrMsg(w
                        + " is unexpected, expect 'D','GW','XZ','R' or 'U' ; calcSimpleRole ");
                return null;
            }
            w = ecc.getAWord();
        }
    ...
    private static Set<String> calcSingleExp(ExpCalcContext ecc) {
        ...
        while (users.size() == 0) {
            ...
            if (",".equals(w))
                ...
                ecc.setLastErrMsg(w
                        + " is unexpected, expect ',' or ')' ; calcSingleRoleExp");
                return null;
            }
        }
    ...
    private static Set<String> calcItemExp(ExpCalcContext ecc) {
        ...
        if ("(".equals(w)) {
            ...
            if (")".equals(w))
                ...
                ecc.setLastErrMsg(w
                        + " is unexpected, expect ')' ; calcItemRole .");
                return null;
            }
            // }else if("D".equals(w)){
    ...
    private static Set<String> calcRolesExp(ExpCalcContext ecc) {
        ...
        while (true) {
            ...
            if ("|".equals(w) || "||".equals(w)) { // 并
                ...
                ecc.setLastErrMsg(w
                        + " is unexpected, expect '||','&&','!',',' or ')' ; calcRoles .");
                return null;
            }
        }
    }

    public static Set<String> calcOptsByRoleExp(String roleExp,
            String lastSameNodeUnit, String userUnit, String previousNodeUnit,
            String nodeUnit, String flowUnit, String creatorCode,
            String userCode, SysVariableTranslate varTrans) {
        if (roleExp == null)
            return null;
        ...
        Integer rank = CodeRepositoryUtil.getUserUnitXzRank(creatorCode,
                flowUnit);
        if (rank != null)
    ...
    public static Set<String> calcOperators(String roleExp,
        ...
        return calcOptsByRoleExp(roleExp, lastSameNodeUnit, userUnit,
                previousNodeUnit, nodeUnit, flowUnit, creatorCode, userCode,
                varTrans);
    }

    public static Set<String> calcOperators(String roleExp, String userUnit,
            String nodeUnit, String flowUnit, String creatorCode,
            String userCode, SysVariableTranslate varTrans) {

        return calcOptsByRoleExp(roleExp, null, userUnit, null, nodeUnit,
                flowUnit, creatorCode, userCode, varTrans);
    }

    public static Set<String> calcOperators(String roleExp, String userUnit,
            String userCode, SysVariableTranslate varTrans) {

        return calcOptsByRoleExp(roleExp, null, userUnit, null, null, null,
                null, userCode, varTrans);
    }

    public static Set<String> calcOperators(String roleExp, String nodeUnit,
            String flowUnit, String creatorCode, SysVariableTranslate varTrans) {

        return calcOptsByRoleExp(roleExp, null, null, null, nodeUnit, flowUnit,
                creatorCode, null, varTrans);
    }
