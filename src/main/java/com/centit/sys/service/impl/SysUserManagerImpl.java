...
public class SysUserManagerImpl extends BaseEntityManagerImpl<FUserinfo>
        implements SysUserManager, UserDetailsService,
    ...
    public void saveUserUnit(FUserunit userunit) {
        ...
        if ("O".equalsIgnoreCase(agencyMode.getDatavalue())
                || "D".equalsIgnoreCase(agencyMode.getExtracode())) {
            FUserunit pUserUnit = sysunitdao.getUserPrimaryUnit(userunit
                    .getUsercode());
            if (null != pUserUnit) {
        ...
        if (userunit.getIsprimary() != null
                && "T".equals(userunit.getIsprimary())) {
    ...
    /*
     * public void saveUserWithPrimaryUnit(FUserinfo userinfo, FUserunit
     * unitinfo) { sysuserdao.saveObject(userinfo);
    ...
    public int deleteUserrole(FUserrole userrole) {
        ...
        if (userrole.getObtaindate().after(DatetimeOpt.truncateToDay(today))) {
            ...
            return 1;
        } else if (userrole.getSecededate() == null
                || userrole.getSecededate().after(today)) {
            userrole.setSecededateToToday(); // .setSecededate(
                                             // java.util.Date.valueOf( (new
    ...
    public void saveUserrole(FUserrole userrole) {
        FUserrole desobj = getValidUserrole(userrole.getUsercode(),
                userrole.getRolecode());
        if (desobj != null) {
            if (desobj.getObtaindate().after(
    ...
    public String getNextUserCode(char cType) {
        String sKey = "00000000000"
                + sysuserdao.getNextValueOfSequence("S_USERCODE");
     ...
     * (non-Javadoc)
     * 
     * @see com.centit.sys.service.SysUserManager#listUnderUnit(java.util.Map,
     * com.centit.core.utils.PageDesc)
     */
    @Override
     ...
     * 
     * @see
     * com.centit.sys.service.SysUserManager#getUserUnderUnit(java.lang.String)
     */
    @Override
    ...
    public void saveBatchUserRole(String rolecode, List<String> ucs) {
        ...
        if (CollectionUtils.isEmpty(userroles)) {
            for (String usercode : userCodes) {
                FUserrole userrole = new FUserrole(new FUserroleId(usercode,
                        rolecode, new Date()));
            ...
            for (FUserrole userrole : userroles) {
                if (!userCodes.contains(userrole.getId().getUsercode())) {
                    // 已经不存在于当前角色中
                    if (userrole.getObtaindate().after(
                            DatetimeOpt.truncateToDay(now))) {
                        userRoleDao.deleteObject(userrole);
                    } else if (userrole.getSecededate() == null
                            || userrole.getSecededate().after(now)) {
                        userrole.setSecededateToToday();
                        userRoleDao.saveObject(userrole);
                    }
                } else if (null == userrole.getSecededate()
                        || userrole.getSecededate().after(now)) {
                    // 存在于当前角色中，不再进行保存
    ...
    public static FDatadictionary getAgencyMode() {
        List<FDatadictionary> sysparam = CodeRepositoryUtil
                .getDictionaryIgnoreD("SYSPARAM");
