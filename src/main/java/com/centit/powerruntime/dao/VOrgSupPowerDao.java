...
public class VOrgSupPowerDao extends BaseDaoImpl<VOrgSupPower> {
    ...
    public Map<String, String> getFilterField() {
        if (filterField == null) {
            ...
            filterField.put(CodeBook.ORDER_BY_HQL_ID, " QL_DEPID,item_Id ");
        }
        return filterField;
    }

    public String getFlowCodeListByItemIdOrgId(String itemId, String org_id) {
        String flowCode = "";
        ...
        if (sqclList != null && !sqclList.isEmpty()) {
    ...
    public VOrgSupPower getSupPowerInfo(String item_id, String orgid) {
        List<VOrgSupPower> po = (List<VOrgSupPower>) getHibernateTemplate()
                .find("From VOrgSupPower where itemId=? and orgId=? ",
                        new String[] { item_id, orgid });
    ...
    public VOrgSupPower getSuppowerInfo(String item_id) {
        List<VOrgSupPower> po = (List<VOrgSupPower>) getHibernateTemplate()
                .find("From VOrgSupPower where itemId=? ",
                        new String[] { item_id });
        if (po != null && po.size() > 0) {
            return po.get(0);
        }
    ...
    public List<VOrgSupPower> OrgSuppowerlist(Map<String, Object> filterMap,
            PageDesc pageDesc) {
        String sql = "From VOrgSupPower where orgId is not null";
        return this.listObjects(sql, filterMap, pageDesc);
    }
