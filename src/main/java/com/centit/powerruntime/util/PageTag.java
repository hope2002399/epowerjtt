...
public class PageTag extends SimpleTagSupport {
	...
	public void setPage(Page page) {
		this.page = page;
		this.offset = page.getStart();
		this.size = page.getTotalCount();
	 ...
	 /**
	  ...
	  public void doTag() throws JspException, IOException {
		    ...
		    if (size > length) {
		    	...
		    	if (offset > 0) {
		    		pageNavigation += "<a href='" + action + pref + "offset=0'>首页</a>\n" +
		            	"<a href='" + action + pref + "offset=" + (offset - length) +
		            	"'>前一页</a>\n";
		    	}
		    	...
		    	long radius = Constants.MAX_PAGE_INDEX / 2 * length;
		    	//如果当前的offset值小于半径
		    	...
		    	for (long i = startOffset;
		    		...
		    		if (i == offset) {
		    			...
		    			pageNavigation += "<a href='" + action + pref + "offset=" + i + "'>" + "[" + 
		    							(i / length + 1) + "]</a>\n";
		    		}
		    	}
		    	...
		    	if (offset < size - length) {
		    		pageNavigation += "<a href='" + action + pref + "offset=" +
		            	(offset + length) + "'>后一页</a>\n" +
		            	"<a href='" + action + pref + "offset=" + lastPageOffset() +
		            	"'>末页</a>\n";
		    	}
		    	// //;
		    	// //;
		    ...
		    pageNavigation =  "共有" + this.size + "条记录, 每页显示" + this.length + "条，共分" + this.pageCount() + "页 " + pageNavigation;
