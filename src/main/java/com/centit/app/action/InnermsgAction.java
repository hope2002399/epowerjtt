...
public class InnermsgAction extends BaseEntityExtremeAction<Innermsg> {
    ...
    public String list() {
        ...
        if (isReveive != null && isReveive.equals("S")) {
            filterMap.put("sender",
                    ((FUserDetail) this.getLoginUser()).getUsercode());
        } else {
            filterMap.put("receive",
                    ((FUserDetail) this.getLoginUser()).getUsercode());
        }
    ...
    public String save() {
        FUserDetail fuser = ((FUserDetail) this.getLoginUser());
        if (object.getReceivetype().equals("O")) {
            object.setReceive(object.getReceive().substring(2));// （initValue产生逗号）将部门前面产生的逗号和空格给截取掉。
        }
