...
public class WorkCalendarImpl implements WorkCalendar {
    ...
    public WorkTimeSpan getWorkTime(Date beginTime, Date endTime) {
        ...
        long betime = DatetimeOpt.getHour(beginTime) * 60 * 60 * 1000
                + DatetimeOpt.getMinute(beginTime) * 60 * 1000;
        long entime = DatetimeOpt.getHour(endTime) * 60 * 60 * 1000
                + DatetimeOpt.getMinute(endTime) * 60 * 1000;
    ...
    public Date getWorkDate(Date baseWorkDate, WorkTimeSpan workTime) {
        ...
        long zctime = m - DatetimeOpt.calcWeekendDays(baseWorkDate, lsdate)
                - holidays + adddays;
        endWorkDate = lsdate;
        if (d > zctime) {
            ...
            while (i < needAddedDays) {
                endWorkDate = new Date(endWorkDate.getTime() + 24 * 60 * 60
                        * 1000);
        ...
        if (workTime.getHours() != 0 || workTime.getMinutes() != 0) {
            // 不超过一天
            if (((DatetimeOpt.getHour(baseWorkDate) * 60 + DatetimeOpt
                    .getMinute(baseWorkDate)) * 60 * 1000 + (workTime
                    .getHours() * 60 + workTime.getMinutes()) * 60 * 1000) < 24 * 60 * 60 * 1000) {
                endWorkDate = new Date(endWorkDate.getTime()
                        + (workTime.getHours() * 60 + workTime.getMinutes())
                        * 60 * 1000);
            ...
            // 超过一天
                ...
                if (!isWorkDate(endWorkDate)) {
                    endWorkDate = new Date(
                            endWorkDate.getTime()
                                    + (workTime.getHours() * 60 + workTime
                                            .getMinutes()) * 60 * 1000 + 24
                                    * 60 * 60 * 1000);
                }
            }
        }
    ...
    public boolean isWorkTime(Date workTime) {
        ...
        if (!isWorkDate(workTime)) {
            ...
            int m = DatetimeOpt.getMinute(workTime) + 100
                    * DatetimeOpt.getHour(workTime);
            OaWorkingTime a = oaWorkingTimeDao.getObjectbyDate(workTime);
            if (a == null)// 默认朝九晚五 codefan@sina.com
                return m > 900 && m < 1700;
            if ("1".equals(a.getHasschedule1())) {
                if (m > Integer.parseInt(a.getSchedule1begin())
                        && m < Integer.parseInt(a.getSchedule1end()))
                    return true;
            }
            if ("1".equals(a.getHasschedule2())) {
                if (m > Integer.parseInt(a.getSchedule2begin())
                        && m < Integer.parseInt(a.getSchedule2end()))
                    return true;
            }
            if ("1".equals(a.getHasschedule3())) {
                if (m > Integer.parseInt(a.getSchedule3begin())
                        && m < Integer.parseInt(a.getSchedule3end()))
                    return true;
            }
            if ("1".equals(a.getHasschedule4())) {
                if (m > Integer.parseInt(a.getSchedule4begin())
                        && m < Integer.parseInt(a.getSchedule4end()))
                    return true;
            }
        }
        return false;
