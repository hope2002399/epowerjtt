...
public class UploadServiceImpl extends BaseEntityManagerImpl<FileInfo>
    ...
    public void upload(File file, String ContentType, String FileName,
        ...
        if (fileInfo.getNeedIndex() == 1) {
            ...
            Indexer.indexFile(
                    sFilePath + "/" + fileInfo.getFileCode() + FileName,
                    // 这个 url 可以通过 finfo.getFileType() 和 finfo.getOptCode() 来生成
                    sFilePath, // TODO url
                    fileInfo.getFileExtName(), fileInfo.getFileType(),
                    fileInfo.getRecorder());
        }

    }

    // 上传到数据库中
    public void uploaddb(File file, String ContentType, String FileName,
            String savePath, FileInfo fileInfo) throws Exception {
