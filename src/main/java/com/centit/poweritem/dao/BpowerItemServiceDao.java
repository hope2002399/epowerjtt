...
public class BpowerItemServiceDao extends BaseDaoImpl<BpowerItemService> {
    private static final long serialVersionUID = 1L;
    public static final Log log = LogFactory.getLog(BpowerItemServiceDao.class);
    ...
    public List<BpowerItemService> getPicList(Map<String, String> filterMap,
            PageDesc pageDesc) {
        List<BpowerItemService> list = new ArrayList<BpowerItemService>();
        ...
        sql.append(" select item_id as itemId, item_Name as itemName,org_id as orgId, ITEM_TYPE as itemType, max(version) as version, ql_state  as qlState"
                + " from v_poweritemserviceinusing where 1=1 ");
        ...
        list = (List<BpowerItemService>) sqlQuery.setResultTransformer(
                Transformers.aliasToBean(BpowerItemService.class)).list();
    ...
    public List<BpowerItemService> getPicListByState(
        ...
        sql.append(" select item_id as itemId, item_Name as itemName,org_id as orgId, ITEM_TYPE as itemType, version as version, ql_state  as qlState"
                + " from b_power_item_service left join V_HI_UNITINFO on  b_power.org_id = V_HI_UNITINFO.depno "
                + " where 1=1  and (item_id ,version) in (select item_id as itemId,    max(version) as version from b_power_item_service where 1 = 1 group by item_id ) ");
    ...
    public BpowerItemService getBpowerItemService(String itemId, Long version) {
        if (version != null) {
            List<BpowerItemService> procs = this
                    .listObjects("From BpowerItemService where item_id =  "
    ...
    public BpowerItemService getSuppowerLastVersion(String itemId,
            Date applyDate) {
        List<BpowerItemService> procs = this
                .listObjects("From BpowerItemService where item_id =  "
                        + HQLUtils.buildHqlStringForSQL(itemId)
    ...
    public List getVersionList(String itemId) {
        ...
        StringBuffer sql = new StringBuffer();
        sql.append(" select version from B_power_item_service where 1=1 ");
        if (StringUtils.isNotBlank(itemId)) {
    ...
    public List<BpowerItemService> getlistSuppowerOld(String itemId,
        ...
        sql.append("select t.item_id as itemId,t.version as version,t.item_name as itemName, t.org_id as orgId,t.begin_time as beginTime, t.end_time as endTime "
                + "from b_power_item_service t where 1=1 "
                + " and version<>0  ");
        if (StringUtils.isNotBlank(itemId)) {
            sql.append(" and item_id = '" + itemId + "' ");
        }
    ...
    public List<BpowerItemService> getlistSuppower(String item_type) {
        ...
        sql.append("select t.item_id as itemId,t.version as version,t.item_name as itemName, t.org_id as orgId,t.begin_time as beginTime, t.end_time as endTime "
                + "from b_power_item_service t where 1=1 "
                + " and version<>0 and (item_id ,version) in (select item_id as itemId,    max(version) as version from b_power_item_service where 1 = 1 group by item_id )  ");
     ...
     */
    @SuppressWarnings("unchecked")
    public List<BpowerItemService> listSuppower4Select(String itemType) {
        List<BpowerItemService> list = new ArrayList<BpowerItemService>();
    ...
    public void updateSuppower(BpowerItemService suppower) {
        doExecuteHql(
                "update BpowerItemService set lastmodifytime=?,qlState=?,endTime=? where cid.itemId =?  and cid.version=? ",
    ...
    public BpowerItemService getSuppowerQlfb(String itemId) {
        List<BpowerItemService> procs = new ArrayList<BpowerItemService>();
        StringBuffer sql = new StringBuffer();
        ...
        sqlQuery.addScalar("version", LongType.INSTANCE);
        sqlQuery.addScalar("orgId", StringType.INSTANCE);
        procs = (List<BpowerItemService>) sqlQuery.setResultTransformer(
                Transformers.aliasToBean(BpowerItemService.class)).list();
        if (procs.size() >= 1) {
            return procs.get(0);
        }
    ...
    public boolean isPowerExist(String itemId) {
        String shql = "select cid.itemId from BpowerItemService where cid.itemId="
                + HQLUtils.buildHqlStringForSQL(itemId);
        List<BpowerItemService> list = this.listObjects(shql);
    ...
    public List getxzlist(String begintime, String endtime) {
        ...
        sql.append("  from b_power_item_service a");
        sql.append(" where a.version = (select max(b.version) as version from b_power_item_service b where b.item_id = a.item_id)");
    ...
    public List<BpowerItemService> getSuppowernum(String item_type) {
        ...
        sql.append("select sum(case when b.item_type is not null then 1 else 0 end) as allnum,"
                ...
                + " from b_power_item_service b"
                + " where 1=1  and version<>0 and end_time is null and (item_id ,version) in (select item_id as itemId,  max(version) as version from b_power_item_service where 1 = 1 group by item_id )");
        SQLQuery sqlQuery = getSession().createSQLQuery(sql.toString());
        versionList = sqlQuery.list();
        return versionList;
    }
