...
public class PcpunishStandardManagerImpl extends
        BaseEntityManagerImpl<PcpunishStandard> implements
        PcpunishStandardManager {
    private static final long serialVersionUID = 1L;
    public static final Log log = LogFactory
            .getLog(PcpunishStandardManager.class);
    ...
    public PcpunishStandard getObjectByItemsId(String itemId, Long version,
            String punishtypeid) {
        return pcpunishStandardDao.getObjectByItemsId(itemId, version,
                punishtypeid);
    }
    ...
    public String getpunishItemType(String punishobjectid, String item_id,
        ...
        String punlishType = "";
        // 获取处罚标准
        List<PcpunishStandard> typeList = pcpunishStandardDao.listPcTypeInUse(
                item_id, version);
                    ...
                    if (!"-".equals(PunishMax) && !"-".equals(PunishMin)
                        ...
                        if ("∞".equals(PunishMax)) {
                            if (dicPunishtypeid1.equals(PunishTypeID)) {
                                ...
                                tmp = tmp
                                        + PunishMin
                                        + (StringUtils.isBlank(pcType
                                                .getLowlimitUnit()) ? pcType
                                                .getToplimitUnit() : pcType
                                                .getLowlimitUnit()) + "以上";
                            }
                        } else {
                            if (null != PunishMax
                                ...
                                if (dicPunishtypeid1.equals(PunishTypeID)) {
                                    tmp = tmp + PunishMin + "元-" + PunishMax
                                            + "元";
                                } else if (dicPunishtypeid2
                                        .equals(PunishTypeID)) {
                                    tmp = tmp + PunishMin + "天-" + PunishMax
                                            + "天";
                                } else {
                                    tmp = tmp
                                            + PunishMin
                        ...
                        // 相等的情况
                    if (StringUtils.isNotBlank(baseToplimit)
                            && StringUtils.isNotBlank(baseLowlimit)
                            && Double.parseDouble(baseToplimit) == Double
                                    .parseDouble(baseLowlimit)) {
                        tmp = tmp + ":" + baseToplimit + "倍";
                    } else {
                        //
                        tmp = tmp + ":" + baseLowlimit + "倍-" + baseToplimit
                                + "倍";
                    }
