 ...
 * @version
 */
public class PcfreeumpiretypeAction extends
        BaseEntityExtremeAction<Pcfreeumpiretype> implements
        ServletResponseAware {
    private static final Log log = LogFactory
            .getLog(PcfreeumpiretypeAction.class);
    ...
    public void setPcfreeumpiretypeList(
            List<Pcfreeumpiretype> pcfreeumpiretypeList) {
    ...
    public void setPcfreeumpiretypeMag(
            PcfreeumpiretypeManager pcfreeumpiretypeMag) {
    ...
    public String list() {
        ...
        pcfreeumpiredegree = pcfreeumpiredegreeManager
                .getObjectByItemsId(object.getDegreeno()); // 获取违法事实程度
        return LIST;
    }
    ...
    public String viewpcfreeumpiretype() {
        ...
        if ("1".equals(isPcpunishStandard)) {
            suppower = suppowerManager.getObjectById(itemId, version);
            PcfreeumpiretypeList = pcfreeumpiretypeMag
                    .listFreeumpiretype(object.getDegreeno());
        }
        return VIEW;
    ...
    public String edit() {
        ...
        if (isPcpunishStandard.equals("1")) {
            ...
            List<Map> fdslist = pcpunishStandardManager.listPunishType(itemId,
                    version, object.getDegreeno().toString());
            this.getSelectList(flowDescribesList, fdslist, 22);
            pcfreeumpiredegree = pcfreeumpiredegreeManager
                    .getObjectByItemsId(object.getDegreeno()); // 获取违法事实程度相关信息放到页面隐藏
                                                               // 用于保存
        }
    ...
    private void getSelectList(List flowDescribesList, List<Map> fdslist,
            int len) {
