...
public class PcpunishItemAction extends BaseEntityExtremeAction<PcpunishItem> {
    ...
    public void setPcpunishItemList(
            @SuppressWarnings("rawtypes") List pcpunishItemList) {
    ...
    @Override
    public String save() {
        PcpunishItem pcpunishItem = pcpunishItemManager
                .getObjectByItemId(object.getPunishclasscode());
        ...
        if (pcpunishItem != null) {
            pcpunishItemManager.copyObjectNotNullProperty(pcpunishItem, object);
            object = pcpunishItem;

        } else {
            // 生成处罚项目类别编号 PDM里为8位
            object.setPunishclassid(pcpunishItemManager
    ...
    public String listPcpunishItem() {
        ...
        pcpunishItemList = pcpunishItemManager.listPcpunishItem(filterMap,
                pageDesc);
