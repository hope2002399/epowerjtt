...
public class PcfreeumpiredegreeDao extends BaseDaoImpl<Pcfreeumpiredegree> {
    ...
    public List<Pcfreeumpiredegree> getObjectByItemsId(String itemId) {
        ...
        return (List<Pcfreeumpiredegree>) this
                .findObjectsByHql(baseHQL, itemId);
    }

    public Pcfreeumpiredegree getDgreeNo(Long degreeno, String itemId,
            Long version) {

        PcfreeumpiredegreeId cid = new PcfreeumpiredegreeId();
        cid.setDegreeno(degreeno);
        String baseHQl = "From Pcfreeumpiredegree  a  where   cid.degreeno='"
                + degreeno + "' and  a.itemId='" + itemId
                + "' and  a.version ='" + version + "' and 1=1";
        List<Pcfreeumpiredegree> procs = this.listObjects(baseHQl);
        if (procs == null || procs.size() < 1) {
            return null;
        } else {
            return procs.get(0);
