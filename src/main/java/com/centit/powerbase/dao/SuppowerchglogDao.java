...
public class SuppowerchglogDao extends BaseDaoImpl<Suppowerchglog> {
    ...
    public Suppowerchglog getObjectByIdAndVersion(String itemId, String version) {
        List<Suppowerchglog> procs = this
                .listObjects("From Suppowerchglog where item_id =  "
    ...
    /**
     * 下面是修改版本信息，TODO
     * 这儿有sql注入问题，我修改了一下可能需要测试，这个SQL语句这是访问一个表，应该可以通过Hql语句来，试试看我注释1中的HQL语句
     * 
     * @param filterMap
     * @param pageDesc
     * @return
    ...
    public List<Suppowerchglog> getlistVersionByItemid(
        ...
        /*
         * List<Suppowerchglog> list = new ArrayList<Suppowerchglog>(); Object
         * itemid = filterMap.get("itemId"); StringBuffer sql = new
         * StringBuffer(); sql.append(
         * "select t.change_id as changeId,t.item_id as itemId,t.version as version,t.chg_reason as chgReason,"
         * +
         ...
         * + " from B_powerchglog t where t.version <> '0' "); if(itemid !=
         * null){ sql.append(" and item_id =?");// + itemid + "' "); }
         * sql.append(" order by t.version,t.change_id");
         * 
         * list = (List<Suppowerchglog>) findObjectsBySql(sql.toString(),
         * itemid, Suppowerchglog.class); SQLQuery sqlQuery =
         ...
         * sqlQuery.addScalar("replyContent", StringType.INSTANCE); list =
         * (List<Suppowerchglog>)
         * sqlQuery.setResultTransformer(Transformers.aliasToBean
         * (Suppowerchglog.class)).list(); pageDesc.setTotalRows(new
         * Integer(list.size())); return list;
         */
    }
