...
public class PcpunishStandardDao extends BaseDaoImpl<PcpunishStandard> {
    ...
    public PcpunishStandard getObjectByClassId(String itemId, Long version) {
        List<PcpunishStandard> procs = this
                .listObjects("From PcpunishStandard where itemId =  "
    ...
    public List<PcpunishStandard> listPcType(String itemId, Long version) {
        ...
        return (List<PcpunishStandard>) findObjectsByHql(baseHQL, new Object[] {
                itemId, version });
    }

    @SuppressWarnings("unchecked")
    public List<PcpunishStandard> listPcpunishStandardByItemIdAndMaxVesion(
            String itemId) {
    ...
    public List<PcpunishStandard> listPcTypeInUse(String itemId, Long version) {
        ...
        List<PcpunishStandard> retlist = (List<PcpunishStandard>) findObjectsByHql(
                baseHQL, new Object[] { itemId, String.valueOf(version) });
    ...
    public List<Map> listPunishType(String itemId, Long version, String degreeno) {
        ...
        String sSqlsen = "select f.datacode as id,f.datavalue as value from F_DATADICTIONARY f "
                + "where  f.datacode not in (select punishtypeid from B_free_umpire_type where  B_free_umpire_type.degreeno="
                + HQLUtils.buildHqlStringForSQL(degreeno)
                + ")"
                + "and f.catalogcode='punishType' and f.datacode in (select punishtypeid from B_punish_Standard where item_Id="
                + HQLUtils.buildHqlStringForSQL(itemId)
                + " and  version= "
                + HQLUtils.buildHqlStringForSQL(Long.toString(version)) + ")";
