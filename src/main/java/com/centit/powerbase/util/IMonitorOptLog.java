...
public interface IMonitorOptLog {
     ...
     * @param tagId
     *            一般用于关联到业务主体[业务主体主键] 如记录办件编号、投诉编号、预报警编号、督查督办编号等 针对联合主键用"#"分隔
     * @param optMethod
     *            记录系统日志的方法名
     * @param optContent
     *            记录操作内容
     ...
     * @param optLog
     */
    void log(Superviselog superviselog);

    List<Superviselog> listMonitorOptLog(Map<String, Object> filterMap,
            PageDesc pageDesc);

    List<Superviselog> listMonitorOptLog(Map<String, Object> filterMap);

    // 增加法制监督操作日志接口：logFZ
    void logFZ(String usercode, String optId, String tagId, String optMethod,
            String optContent, String oldValue, String bjType);

    // 缺失optId：非功能性日志记录
    void logFZ(String usercode, String tagId, String optContent,
            String oldValue, String bjType);

    // 缺失oldValue：新增操作日志记录
    void logFZ(String usercode, String tagId, String optContent, String bjType);
}
